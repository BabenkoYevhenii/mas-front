...
class OrderWrapper extends Component {
    ...
    render() {
        ...
        const {order, tripName, tripDate} = this.props;
        ...
        return (
            <Segment raised>
                <Segment>
                    <Segment vertical>
                        <Header as={"h1"}>Trip: {tripName} | Date: {tripDate}</Header>
                    </Segment>
                    ...
                    <Table padded='very' basic='very' celled>
                        ...
                        <Table.Body>
                            {tickets.map(ticket => <Table.Row
                                key={`order: ${id} ticket: ${ticket.id} suname: ${ticket.passenger.surname} documentNumber: ${ticket.passenger.documentNumber}`}>
                                <Table.Cell>{ticket.placeNumber}</Table.Cell>
                                <Table.Cell>{ticket.type}</Table.Cell>
                                <Table.Cell>{ticket.passenger.name}</Table.Cell>
                                <Table.Cell>{ticket.passenger.surname}</Table.Cell>
                                <Table.Cell>{ticket.passenger.documentType}</Table.Cell>
                                <Table.Cell>{ticket.passenger.documentNumber}</Table.Cell>
