...
class OrderPage extends Component {
    ...
    async componentDidMount() {
        ...
        const [trip, attenders] = await Promise.all([tripApi.getTrip(tripId), orderApi.getAllUserAttenders()]);

        console.log(attenders);
        this.setState({trip, attenders, loading: false});
    }
    ...
    chooseAttenders = () => {
        const {pickedAttenders, trip, ticketType: type} = this.state;
        const {price, name: tripName, dateTime: tripDate, id: tripId} = trip;
    ...
    getAddAttenderModal = () => {
        ...
        return (
            <Modal open={addAttenderModalOpen} size='small' dimmer='blurring'>
                <Modal.Header><Header as='h3' color='blue' content={`Add Passenger`}/></Modal.Header>
                <Modal.Content>
                    <Form>
    ...
    confirmOrder = async () => {
        const {orderData, trip} = this.state;

        const confirmedOrder = await orderApi.createOrder(trip.id, orderData);
    ...
    render() {
        const {trip, attenders, pickedAttenders, loading, orderData, addAttenderModalOpen, ticketType, ticketTypeChosen, attendersChosen, orderConfirmed} = this.state;
        ...
        return (
            ...
            <Container>
                ...
                <Container>
                    ...
                    {!ticketTypeChosen && <Container text>
                        <Form onSubmit={() => this.setState({ticketTypeChosen: true})} loading={loading}>
                            ...
                            <Form.Dropdown id='ticketType' name='ticketType' onChange={this.onTicketTypeChange}
                                           ...
                                           options={[
                                               {key: 0, value: "FIRST_CLASS", text: "First class"},
                    ...
                    {trip && attendersChosen &&
                    <div>
                        <OrderWrapper order={orderData} tripName={trip.name} tripDate={trip.dateTime}
